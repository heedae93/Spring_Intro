...
### 정적 컨텐츠
- 스프링은 타임리프 같은 동적 html말고 정적 컨텐츠 제공 기능도 있다.
- 브라우저에서 요청한 주소를 톰캣이 받고 스프링 컨테이너에 전달한다.
- 해당 주소가 스프링 컨테이너에 등록이 되어 있다면 타임 리프같은 동적 html리턴 하겠지만
없으면 resources/static에서 해당 경로를 찾아서 있으면 반환한다.
